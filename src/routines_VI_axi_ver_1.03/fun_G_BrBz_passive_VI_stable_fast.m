function [G_Br_VI,G_Bz_VI] = fun_G_BrBz_passive_VI_stable_fast(tri, ...
    nodes, ...
    N_order, ...
    degree_G_source, ...
    n_G_source, ...
...
Green_Mat_Gauss_Br = zeros(npt_point,n_G_source*nt);
Green_Mat_Gauss_Bz = zeros(npt_point,n_G_source*nt);
...
for ii = 1:n_G_source*nt
    ...
    if RUN_MEX
        ...
        [ vec_Br_all,vec_Bz_all ] = fun_Green_BrBz_Loop( source, point, I_source);
        
    end
    
    Green_Mat_Gauss_Br(:,ii) = vec_Br_all;
    Green_Mat_Gauss_Bz(:,ii) = vec_Bz_all;
...
G_Bz_VI = fun_assemby_G_VI_mex(tri, ...
    nn, ...
    n_G_source, ...
    Green_Mat_Gauss_Bz, ...
    det_Jac, ...
...
%%
clear Green_Mat_Gauss_Br Green_Mat_Gauss_Bz
