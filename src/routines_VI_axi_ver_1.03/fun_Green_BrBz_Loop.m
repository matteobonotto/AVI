...
% %     tmp_bz = 4.d-7*pi*(source(:,1).^2).'./(2*(r0^2+(ZZ(ind_axis)-(source(:,2).^2).').^2).^1.5);
% %     bz(ind_axis) = sum(tmp_bz,2);
    for  jj=1:ncoil
        r0=source(jj,1);
        z0=source(jj,2);
        curr0 = curr(jj);
        tmp_bz = 4.d-7*pi*r0^2./(2*(r0^2+(ZZ(ind_axis)-z0).^2).^1.5);
        bz(ind_axis)= bz(ind_axis) + tmp_bz*curr0;
    end
end
